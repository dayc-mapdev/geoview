...
function PanelContent(props: TypePanelContentProps): JSX.Element {
  ...
  useEffect(() => {
    setLegend(
      api
        .map(mapId!)
        .legend.createLegend({ layerIds: mapLayers, isRemoveable: true, canSetOpacity: true, canZoomTo: true, expandAll: isExpandAll })
    );
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isExpandAll]);
  ...
  useEffect(() => {
    setLegend(
      api
        .map(mapId!)
        .legend.createLegend({ layerIds: mapLayers, isRemoveable: true, canSetOpacity: true, canZoomTo: true, hideAll: isHideAll })
    );
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isHideAll]);
